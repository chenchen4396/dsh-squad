...
export async function apply(ctx: Context, config: AgentTeamConfig): Promise<void> {
  ...
  try {
    ...
    // Warm the model directory while nobody is waiting: the first view to open
    // otherwise pays for one network round trip per provider.
    void service.catalog().catch(() => undefined)
  } catch (error) {
    transport?.dispose()
    await service?.disposeWorkspaceTracking()
    await assistantBuilderRuntime?.dispose()
