...
export class AgentTeamService extends Service {
  ...
  exportBundle(input: { teamIds?: readonly string[] | undefined } = {}): SquadBundle {
    ...
    const ruleDocuments = this.store.listRuleDocuments()
      .filter(document => wanted === undefined || this.documentIsUsed(document.id))
      .sort((left, right) => left.path.localeCompare(right.path))
    const ruleKeys = new Map(ruleDocuments.map(document => [document.id, `rule:${document.path}`]))
    const assistants = this.store.listAssistants()
  ...
  private documentIsUsed(documentId: string): boolean {
    return this.store.listAssistants().some(assistant => assistant.ruleDocumentAllowlist.includes(documentId))
  }
  ...
  async importBundle(raw: unknown): Promise<BundleImportSummary> {
    ...
    for (const document of bundle.ruleDocuments) {
      const existing = mode === 'overwrite'
        ? this.store.listRuleDocuments().find(current => current.path === document.path)
        : undefined
      if (existing !== undefined) {
        await this.store.putRuleDocument({
          ...existing,
          title: document.title,
          fileName: document.fileName,
          content: document.content,
    ...
    for (const assistant of bundle.assistants) {
      ...
      const existing = mode === 'overwrite'
        ? this.store.listAssistants().find(current => current.name === assistant.name)
        : undefined
      if (existing !== undefined) {
        await this.store.putAssistant({
          ...existing,
